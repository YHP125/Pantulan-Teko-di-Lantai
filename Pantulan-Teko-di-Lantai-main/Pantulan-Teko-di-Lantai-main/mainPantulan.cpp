...
void gambarTV() {
    ...
    // TV dimensions (rectangular prism, longer width, vertical orientation)
    float xMin = 7.5f, xMax = 10.5f;  // Width (3 units, longer)
    float yMin = 0.0f, yMax = 2.0f;   // Height (2 units, shorter)
    float zMin = 11.0f, zMax = 13.0f; // Depth (2 units, same as height)
    ...
    // Stand
    float standWidth = 1.0f; // Wider for stability
    float standHeight = 0.3f; // Thicker stand
    float standDepth = 0.8f; // Slightly less than depth
    float standX = (xMin + xMax) / 2.0f - standWidth / 2.0f;
    float standY = yMin - standHeight;
    float standZ = (zMin + zMax) / 2.0f - standDepth / 2.0f; // Centered along z-axis
